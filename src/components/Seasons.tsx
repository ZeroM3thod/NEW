...
const pastSeasons = [
  {tag:'Completed',name:'Season One',  period:'Jan 2023 — Apr 2023',roi:'+18.2%',roiLabel:'Final ROI',  details:[{label:'Total Pool',value:'$12M'},{label:'Investors',value:'8,400'},{label:'Duration',value:'90 Days'}]},
  {tag:'Completed',name:'Season Two',  period:'Jun 2023 — Sep 2023',roi:'+23.7%',roiLabel:'Final ROI',  details:[{label:'Total Pool',value:'$31M'},{label:'Investors',value:'19,200'},{label:'Duration',value:'90 Days'}]},
  {tag:'Running',  name:'Season Three',period:'Nov 2023 — Feb 2024',roi:'+24–32%',roiLabel:'Expected ROI',details:[{label:'Total Pool',value:'$57M'},{label:'Investors',value:'34,800'},{label:'Duration',value:'90 Days'}]},
];
      ...
      <section className="seasons" id="seasons">
        <div className="seasons-inner">
          <div className="seasons-header reveal">
            ...
            <button className="btn-primary" onClick={() => router.push('/signup')}>Join Season 4</button>
          </div>
          <div className="seasons-grid reveal">
            {pastSeasons.map((s,i)=>(
              <div key={i} className="season-card">
                ...
                <div className="season-detail">
                  {s.details.map((d,j)=>(
                    <div key={j} className="season-detail-item"><span>{d.label}</span><strong>{d.value}</strong></div>
                  ))}
                </div>
              </div>
            ))}
            <div className="season-card active season-card--full">
              ...
              <div className="season-detail" style={{marginTop:'20px'}}>
                {[{label:'Min. Entry',value:'$100 USDT'},{label:'Pool Cap',value:'$80M'},{label:'Duration',value:'90 Days'},{label:'Referral Bonus',value:'5% / Withdrawal'}].map((d,i)=>(
                  <div key={i} className="season-detail-item"><span style={{color:'rgba(246,241,233,.4)'}}>{d.label}</span><strong>{d.value}</strong></div>
                ))}
              </div>
            </div>
          </div>
