...
CREATE TABLE IF NOT EXISTS public.profiles (
  ...
  commission_rate    NUMERIC     DEFAULT 7.0,  -- % of referral's profit
...
-- (needed for referral look-up, username checks, admin lists, etc.)
CREATE POLICY "profiles_select_all"
  ON public.profiles FOR SELECT
  USING (true);

-- Users can create their own profile (trigger also runs as SECURITY DEFINER)
...
-- ── investments ──────────────────────────────────────────────
-- Users see their own investments PLUS investments belonging to
-- people they referred (so referral commission is calculable).
...
--    Called by the season investment flow to atomically add to
--    current_pool, avoiding read-modify-write race conditions.
-- ══════════════════════════════════════════════════════════════
...
--    Fires after every INSERT on auth.users and creates the
...
BEGIN
  ...
  -- Guard against rare double-fire (idempotent)
...
-- ══════════════════════════════════════════════════════════════
-- 11. BACKFILL EXISTING DATA (safe to run on populated DBs)
-- ══════════════════════════════════════════════════════════════
...
-- (sets it to created_at so it is already expired = unlocked)
...
-- ══════════════════════════════════════════════════════════════
-- 12. VERIFICATION QUERIES (comment out before final run if needed)
-- ══════════════════════════════════════════════════════════════
...
WHERE  routine_schema = 'public'
  AND  routine_name IN ('is_admin','handle_new_user','increment_season_pool');
